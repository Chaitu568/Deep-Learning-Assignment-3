...
def to_index(ordinal, shape, out_index):
    ...
    # TODO: Implement for Task 2.1.
    # raise NotImplementedError('Need to implement for Task 2.1')
    # get the stride from the defined function
...
def shape_broadcast(shape1, shape2):
    ...
    if len(shape1) != len(shape2):
        if len(shape1) > len(shape2):
            for i in range(abs(len(shape1) - len(shape2))):
                shape2.insert(i, 1)
        ...
        for i in range(len(shape1)):
            ...
            elif shape1[i] == 1 or shape2[i] == 1:
                new_shape.append(max(shape1[i], shape2[i]))
            else:
                raise IndexingError("Failed to broadcast shape")
    else:
        for i in range(len(shape1)):
            if shape1[i] == shape2[i]:
                new_shape.append(shape1[i])
