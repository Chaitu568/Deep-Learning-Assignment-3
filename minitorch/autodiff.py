...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        """
        var_derv = []
        bck_derivatives = cls.backward(ctx, d_output)
        if not isinstance(bck_derivatives, tuple):
            bck_derivatives = wrap_tuple(bck_derivatives)
        for i in range(0, len(inputs)):
            if not is_constant(inputs[i]):
                var_derv.append((inputs[i], inputs[i].expand(bck_derivatives[i])))
        return var_derv
